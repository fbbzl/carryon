...
`qa` 独占生成和维护单元、集成、API、端到端及回归测试资产，执行测试并给出验收结论。`dev` 只修复 Bug 指向的业务实现；`cr` 不生成或执行测试；`dp` 只消费测试结论。
