...
本技能不用于替代正式技术方案协议、架构设计、代码实现或测试。权限、金额、数据、合规和验收规则不会因为被展示就自动确认，其状态仍由 `req` 根据当前来源和反馈判定。
...
4. 只保留会实质影响范围、验收或风险的待决定事项。需要用户反馈时提供选项、影响和简洁反馈入口；用户已明确的内容不重复确认，反馈也不自动执行外部操作。
