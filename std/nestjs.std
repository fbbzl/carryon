...
0. 实现 `OnModuleInit`、`OnApplicationShutdown` 等 Hook 时，启动和清理必须有界且可重复；关闭时先停止接收，再排空或明确拒绝已接收任务，最后释放依赖，并通过 shutdown hook、显式信号处理或平台等效路径触发。
...
2. 后台任务和事件监听器必须有错误观察、关闭和测试路径，部分成功时记录已完成副作用并恢复未完成范围。

## 测试与陷阱

0. 测试沿用项目工具，覆盖 Module 装配、Provider 作用域、请求管线顺序、消息确认前后失败/重复投递/死信、关闭排空和真实持久化边界。
