...
0. 使用受管理的 Executor、`CompletableFuture`、结构化并发或虚拟线程时，以实际 JDK 和运行模型为准，并定义队列边界、饱和与拒绝策略。
1. 保留线程中断状态，超时和取消沿调用链传播；关闭时停止提交、限时等待或排空、处置未完成任务后再释放依赖，多任务失败必须定义汇聚和同级任务策略。
2. 默认不在数据库事务中等待不可控的外部服务；无法避免时限制超时并明确幂等、锁影响、补偿和失败恢复，跨系统一致性优先使用项目既有模式。
3. 静态可变状态、非线程安全格式化器和共享集合必须有清晰同步策略。

## 工具与测试

0. 构建沿用 Maven 或 Gradle；格式化、Checkstyle、SpotBugs、Error Prone 等只使用项目已配置工具。
1. 测试沿用 JUnit、TestNG 或项目既有框架；并发、事务和序列化边界需要相称的集成验证，覆盖拒绝执行、中断/取消、多 Future 失败及支持 JDK/消费者组合。
